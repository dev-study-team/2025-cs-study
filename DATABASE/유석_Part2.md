...
웹 컨테이너 WAS가 실행되면서 일정량의 Connection 객체를 미리 만들어서 pool에 저장했다가, 클라이언트 요청이 오면 Connection 객체를 빌려주고 해당 객체의 작업이 완료되면 다시 Connection 객체를 반납 받아서 pool에 저장하는 프로그래밍 기법입니다.

> 14-1. DB와 Client가 Connection을 어떻게 구성하는지 설명해 주세요.

클라이언트와 DB 사이의 연결은 보통 TCP/IP 프로토콜을 기반으로 연결되며, 스프링 부트에서는 기본적으로 HikariCP를 활용해 Connection을 관리하며, Connection을 효율적으로 재사용하고 네트워크 안정성을 고려하는 것이 중요합니다.

## 📚 15. Table Full Scan, Index Range Scan에 대해 설명해 주세요.
테이블 풀 스캔은 인덱스를 사용하지 않고 테이블에 있는 모든 데이터를 확인하면서 검색을 수행하는 방식입니다. 인덱스 레인지 스캔은 B-Tree 인덱스를 활용하여 특정 범위의 데이터만 검색하는 방식으로 테이블의 모든 행을 읽지 않고 일부 데이터만 빠르게 접근합니다. 이를 통해 검색 조건에 따라 특정 범위를 탐색하여 검색 성능이 향상됩니다.

> 15-1. 가끔은 인덱스를 타는 쿼리임에도 Table Full Scan 방식으로 동작하는 경우가 있습니다. 왜 그럴까요?

네. 먼저 옵티마이저의 판단에 따라 데이터가 한쪽으로 편향되어 있어 탐색하는 데에 대부분의 데이터를 탐색할 경우 인덱스를 거치는 것보다 바로 풀 테이블 스캔을 하는 것이 랜덤 액세스 발생을 줄일 수 있기 떄문입니다.

또 다른 경우에는 테이블 크기가 작아 테이블 풀 스캔이 더 빠른 경우도 있고, 기존 테이블에 대량의 데이터가 추가 되었는데 아직 통계 정보가 갱신되지 않아 여전히 테이블 풀 스캔이 더 빠르다고 착각하는 경우도 있어, 이 경우에는 통계 정보를 갱신해야 합니다.

> 15-2. COUNT (개수를 세는 쿼리) 는 어떻게 동작하나요? COUNT(1), COUNT(*), COUNT(column) 의 동작 과정에는 차이가 있나요?

COUNT(*)와 COUNT(1)의 경우에는 실제 동작에 차이가 없습니다. 테이블의 NULL값에 상관없이 모든 행의 개수를 세는 방식으로 각 컬럼을 모두 조회하지 않고 레코드 개수만 빠르게 탐색합니다. 반대로 COUNT(column)은 NULL을 제외하고 카운트하기 때문에 결과값이 다릅니다. 

## 📚 16. SQL Injection에 대해 설명해 주세요.
SQL 인젝션은 사용자가 입력한 데이터를 검증 없이 SQL 쿼리에 직접 포함할 때 발생하는 보안 취약점입니다. SQL 문을 조작해서 데이터베이스를 무단으로 조회, 변경, 삭제할 수 있는 보안 취약점입니다.

> 16-1. 그렇다면, 우리가 서버 개발 과정에서 사용하는 수많은 DB 라이브러리들은 이 문제를 어떻게 해결할까요?

가장 강력한 방법으로 매개변수 바인딩이 있습니다. SQL쿼리와 사용자 입력을 분리하여 실행하므로, 공격자가 SQL을 조작할 수 없습니다. 즉, 공격자가 입력한 악성 SQL 코드가 그대로 실행되지 않게 하기 위해 단순한 문자 데이터로 인식되도록 변환하는 방식입니다.

다른 방법으로는 SQL을 직접 작성하지 않는 ORM 방식을 통해 인젝션 위험을 낮출 수 있습니다. JPA, MyBatis와 같은 라이브러리는 내부적으로 매개변수 바인딩을 활용하기 때문에 안전합니다.
